...
FITNESS preliminary data (Figure 2B) mean distance to centroid = 0.27 mds, sd = 0.17,  = 0.05, calculated with the pwr package in R36). 
```

The 8-month time point is expected to cluster MTM category, of which NutriCare offers five options (Section 1.5). A sample size of 10 patients is estimated to have 80% power to detect a difference in means of one standard deviation (FITNESS preliminary data (Figure 2B) mean distance to centroid = 0.27 mds, sd = 0.17,  = 0.05, calculated with the pwr package in R36). Roughly equal distribution of patients across MTM categories would have sufficient sample size to test all MTMs.
